...
When run in a Bruker dataset folder (containing a pdata folder and an acqu file), the solvent, nucleus/nuclei, and peaklists of the form pdata/1/peaklist.xml or pdata/1/peak.txt are read automatically. In this case, no options need to be specified. The program will infer the dimensionality from the peak list format.
